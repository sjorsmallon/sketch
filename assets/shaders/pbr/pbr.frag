...
void main() {
    ...
    // Calculate lighting
    vec3 view_dir = normalize(-frag_pos); // does this assume I am at the origin?
    ...
    vec3 ks = vec3(f * metallic + (1.0 - metallic));
    vec3 kd = vec3(1.0 - ks);
    kd *= 1.0 - metallic;

    vec3 specular = vec3((ndf * g * f) / (4.0 * dot(normal_map, view_dir) * dot(normal_map, light_dir)));
